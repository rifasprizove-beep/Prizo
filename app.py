...
# -------- modelos (SIN CAMBIOS, SOLO MEJORO LA VALIDACIÓN DE EMAIL) --------
...
class PaymentRequest(BaseModel):
    ...
    # Añado 'method' para compatibilidad con el frontend
...
# -------- rutas (SIN CAMBIOS EN LAS EXISTENTES, EXCEPTO LAS DE PAGO) --------
@app.get("/health")
async def health():
...
async def reserve(req: ReserveRequest):
    ...
    return {"tickets": svc.reserve_tickets(req.email, req.quantity)}
...
# El frontend ahora llama a /payments/submit, lo ajusto para que siga funcionando con tu endpoint
@app.post("/payments/submit") 
async def submit_payment_unified(req: PaymentRequest):
    try:
        data = svc.create_mobile_payment(req.email, req.quantity, req.reference, req.evidence_url)
...
async def draw_pick(req: DrawPickRequest):
    if req.n < 1: raise HTTPException(400, "n debe ser >= 1")
    draw_id = req.draw_id
